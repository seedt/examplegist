...
== Third Use Case
For a given Outcome show me the average price over the last hour.
...
RETURN outcome.description, collect(price),
(reduce(r = 0, p IN collect(price) | p.value + r) / size(collect(price)))
----
